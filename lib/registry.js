...
module.exports = class ComponentRegistry {
  ...
  alias(name, target) {
    ...
    if (this.aliases[name]) {
      throw new Error('Alias prefix \'' + name + '\' is already in use');
    }
  ...
  singletonBuilder() {
    ...
    return function(path, provider) {

      if (!registry.components[path]) {
        registry.components[path] = registry.build(path, provider);
      }
  ...
  component(path) {
    ...
    try {
      ...
      if (provider.$builder) {
        return provider.$builder(path, provider.$get);
      }

      return this.build(path, provider.$get);
