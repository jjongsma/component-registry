...
export default class ContainerRegistry {
  ...
  get(path) {
    return this.component(this.provider(path));
  }
  ...
  register(path, provider) {
    ...
    if (dependencies.length) {
      ...
      sorted.map((dep) => this.provider(dep));

      // Inject the loaded providers into the builder in argument order
      this.providers[path] = builder.apply(null, dependencies.map((dep) => this.provider(dep)));
  ...
  component(path) {
    ...
    if (dependencies.length) {
      ...
      // Load them in correct dependency order
      return sorted.reduce((promise, dep) => promise.then(() => this.component(dep).then((instance) => {
