 ...
 * };
 *
 * `registration` is an instance of ComponentRegistration which exposes methods for
 * registering a component factory: provider(), factory(), component() and value().
 *
 * `config` is an object that will allow the component to configure itself for the
 * current environment. This is the global config object passed in the registry
 * constructor and is not specific to the component. A common pattern is to pass
 * `process.env` to the registry constructor, but more sophisticated implementations
 * can be provided.
 */
