...
module.exports = class ComponentRegistry {
  ...
  component(path, from) {
    ...
    if (REGISTRY_DEBUG && actual !== path) {
      console.log(this.label('ALIAS', 7) + path + ' -> ' + actual);
    }
    ...
    try {
      ...
      if (provider.$builder) {
        return provider.$builder(actual, provider.$get, from);
      }

      return this.build(actual, provider.$get, from);
