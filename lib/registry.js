...
module.exports = class ComponentRegistry {
    ...
    for (let base of paths) {
      ...
      try {
        ...
        if (err.code !== 'MODULE_NOT_FOUND') {
          throw Object.assign(err, { module: prefix + path});
        }
      }
  ...
  provider(path) {
    ...
    if (!this.providers[path]) {
      ...
      try {
        ...
        register(new ComponentRegistration(this, path), this.config);
  ...
  singletonBuilder() {
    ...
    return function(path, provider, from) {

      if (!registry.components[path]) {
        registry.components[path] = registry.build(path, provider, from);
      }
  ...
  component(path, from) {
    ...
    try {
      ...
      if (provider.$builder) {
        return provider.$builder(path, provider.$get, from);
      }

      return this.build(path, provider.$get, from);
  ...
  notify(evt, path, context) {
    ...
    if (REGISTRY_DEBUG) {
      console.log(this.label(evt, 10) + path + (context.description ? ' ' + context.description : ''));
    }
